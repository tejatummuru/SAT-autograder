...
import {reading_writing, raw_score_math, score, ScoreDisplay} from './ScoreDisplay';
import {
  BrowserRouter,
  Routes,
  Route,
...
function App() {
  return (
     //so the problem is that the forms are stacked on top of each other and when one submits it overrides the sumbit of the other pnes so only the
     //first scale gets saved and used, but also the scores are being calculated correcfly but not displayed in the webpage
    <div className="App">
      ...
      <BrowserRouter>
        ...
        <Routes>
          ...
          <Route path="/score" element={[<h2>Reading Score</h2>,reading_writing, <h2>Math Score</h2>, raw_score_math, <h1>Final Score</h1>, score, <ScoreDisplay/>]}/>
        </Routes>
      </BrowserRouter>
